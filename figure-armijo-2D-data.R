...
for(param.i in 1:nrow(initial.dt)){
  ...
  while({
    ...
    n01 <- function(x)(x-min(x))/(max(x)-min(x))
    norm01 <- function(over, under, f){
      ok <- over==FALSE & under==FALSE
      out <- f
      out[ok] <- n01(f[ok])
      fcase(
        over, Inf,
    ...
    , under := f<min(f[Function=="objective"])
    ][
    , thresh.log10 := norm01(over, under, log10.f)
    ][])
    armijo.only <- armijo.it.dt[step.size %in% step.size.vec]
    armijo.wide <- dcast(
      armijo.only,
      step.size + log2.step ~ Function,
      value.var=c("f", "log10.f", "over","thresh.log10")
    ...
    ggplot()+
      ...
      geom_point(aes(log2.step, log10.f, color=Function), data=armijo.it.dt[under==FALSE & over==FALSE], shape=21, fill=NA)+
      geom_point(aes(log2.step, log10.f_objective, fill=admissible), data=armijo.wide, shape=21)
    ggplot()+
      geom_line(aes(log2.step, thresh.log10, color=Function), data=armijo.it.dt)+
      geom_point(aes(log2.step, thresh.log10, color=Function), data=armijo.it.dt[under==FALSE & over==FALSE], shape=21, fill=NA)+
      geom_point(aes(log2.step, thresh.log10_objective, fill=admissible), data=armijo.wide, shape=21)
...
save(opt.vec.dt, step.dt, armijo.fun.dt, armijo.point.dt, grid.dt, initial.dt, file="figure-armijo-2D-data.RData")
